...
const handleVideoRef = (component, videoIndex, currViewableIndex, isPaused, isFocused) => {
  if (!component) {
    return;
  }
...
const VideoCard = ({videoIndex, currViewableIndex, videoURI}) => {
  ...
  return (
    <Pressable style={{flex: 1}} onPress={() => setIsPaused(!isPaused)} onLongPress={() => console.log("LONG")}>
      <Video
        ref={(component) => handleVideoRef(component, videoIndex, currViewableIndex, isPaused, isFocused)}
        style={{flex : 1}}
        source={{
          uri: videoURI,
        }}
